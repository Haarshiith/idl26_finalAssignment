...
class AlexNet(nn.Module):
    ...
    def __init__(self, **kwargs):   # NOTE (Phase 1/3): ignores in_channels & num_classes
        super().__init__()

        drop_rate = kwargs.get("drop_rate", 0.5)
        
        self.features = nn.Sequential(
            nn.Conv2d(3, 48, kernel_size=7, stride=2, padding=3),   # NOTE: hardcoded 3 channels (data has 1)
            nn.BatchNorm2d(48),
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(2048, 1024),     # NOTE: hardcoded flatten size
            nn.ReLU(inplace=True),
            nn.Dropout(p=drop_rate),
            nn.Linear(1024, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, 11),       # NOTE: hardcoded 11 classes
        )
